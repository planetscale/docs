...
subtitle: 'This document describes the security features of PlanetScaleDB.'
---

## Security features

PlanetScaleDB is a fully managed scaling database solution. The database runs entirely on PlanetScale's private cloud; your application [connects to the database](connecting-to-db) using a standard connection string. Within the PlanetScale private cloud, data is secure at rest on the storage level. In order to secure your application connection to the PlanetScale Database, the server accepts TLS connections. PlanetScaleDB allows you to [configure an IP Access List](allowing-ips).
