...
<InfoBlock type="tip">
Tip: If you run into a proxy ERROR stating that address is already in use, run the following command: <em>spring stop</em>. (Spring is a Ruby library that pre-loads and runs all of our code into memory, detecting when our source code has changed.) Then, try the above command again.
</InfoBlock>
