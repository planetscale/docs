...
<InfoBlock type="tip">
  If you are using PlanetScale with Prisma, go to your database's Settings page in the web app 
  and enable "Automatic schema migrations." Select "Prisma" as the migration framework.
  When enabled, this setting updates the <em>_prisma_migrations</em> table 
  each time you branch with the latest migration. If disabled, running{' '}
  <em>prisma migrate dev</em> will try to run all migrations every time, instead of
  only the latest one.
</InfoBlock>
...
<InfoBlock type="tip">
  When you merge a deploy request in PlanetScale, the <em>_prisma_migrations</em>{' '}
...
Make sure to go into your database's Settings page to turn on "Automatic schema migrations" for Prisma after the database is created. 
...
<InfoBlock type="tip">
  You can learn more about the <em>prisma migrate dev</em>{' '} command in the <a href="https://www.prisma.io/docs/concepts/components/prisma-migrate#development-environments">Prisma docs</a>.
</InfoBlock>
...
1. Delete your previous `shadow` branch:

```zsh
pscale branch delete prisma-playground shadow
```

Since shadow databases are created and deleted automatically each time you run a development-focused Prisma command, we need to also delete shadow _development_ branches in PlanetScale too. 
...
As part of creating that branch, PlanetScale copies _every_ piece of data from `main`’s `_prisma_migrations` table over to your new development branch.

3. Connect to the two branches:
...
4. In another terminal, switch to your newly created development branch, `add-subtitle-to-posts`:

```bash
pscale branch switch add-subtitle-to-posts --database prisma-playground
```

5. In the `/schema/schema.prisma` file, update the `Post` model:
...
6. Create another Prisma migration for your table and call it `add-subtitle-to-posts`:
...
7. Open a deploy request for your `add-subtitle-to-posts` branch, so that you can deploy these changes to `main`.

You can complete the deploy request either in the web app or with the `pscale deploy-request` command.

8. Once the deploy request is merged, you can see the migration in your main branch's `_prisma_migrations` table:
...
Now that you've successfully conducted your first automatic Prisma migration in PlanetScale and know how to handle future migrations, it's time to deploy your application with a PlanetScale database! Let's learn how to [deploy an application with a PlanetScale database to Heroku](/tutorial/deploy-to-heroku).
