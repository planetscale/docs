...
If you're just getting started and still need to set up a database, we recommend starting with the [PlanetScale quickstart guide](/tutorials/planetscale-quick-start-guide) first. We also have language/framework-specific guides under "**Integration guides**" if you prefer a more detailed walk-through.

PlanetScale uses [database branches](/concepts/branching) to create a development-friendly workflow. Your database is initially created with a default branch, `main`, which is meant to serve as a development branch before promoting it to production.

While *developing* your application, you'll need to connect to a *development* branch. Your production application, however, should be connected to your production database branch. Check out our [Branching guide](/concepts/branching) for more information about the branching workflow.
...
![Credentials to connect to PlanetScale](/img/docs/planetscale-branch-credentials.png)
...
The path to the certificate depends on your system. The above example shows the path for MacOS, but you can find others in our [Secure connections documentation](/concepts/secure-connections#ca-root-configuration).
...
If you're **unsure of what to put here**, we recommend selecting your application's language from the dropdown in the PlanetScale dashboard (see step 3 above) and copying the credentials from there. This includes the necessary SSL configuration variables and shows what files they belong in. Additionally, we show you the correct certificate path by default based on your system.

### Generate credentials in the PlanetScale CLI

If you prefer working from the CLI, you can quickly spin up new credentials there. Make sure you have the [CLI set up](/concepts/planetscale-environment-setup) first.
...
1. Make sure you have [the CLI set up](/concepts/planetscale-environment-setup) and then run the following command:
...
Once your application is connected to a development database branch, you're able to make schema changes in an isolated development environment without worrying about affecting production. Additionally, the PlanetScale workflow allows you to making [non-blocking schema changes](/concepts/nonblocking-schema-changes) without locking or causing downtime for production databases.
...
4. Once satisfied and ready to deploy your changes to production, [create a deploy request](/concepts/deploy-requests).
5. You or your team can review and approve the schema changes.
6. Deploy your deploy request to production.
7. Bonus: If you realize you made a mistake, you can click "Revert changes" to [undo a schema change](/concepts/deploy-requests#revert-a-schema-change). 

Note, you must already have [a production branch](/concepts/branching#promote-a-branch-to-production) in place to create a deploy request.
