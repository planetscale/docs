...
This guide will integrate [a simple PHP app](https://github.com/planetscale/php-example) with PlanetScale that will display a list of products stored in the database. If you have an existing application, you can also use that.
...
$mysqli->connect($hostname, $username, $password, $dbName);
...
Fill in `USERNAME`, `PASSWORD`, `ACCESS_HOST_URL` and `DATABASE_NAME` with the corresponding values from the CLI output above.
