...
![PHP sample application homepage](/img/docs/php-sample-app-homepage.png)
...
You can create a database either in the [PlanetScale dashboard](https://app.planetscale.com) or from the PlanetScale CLI. This guide will use the CLI, but you can follow the database setup instructions in the [PlanetScale quick start guide](/tutorials/planetscale-quick-start-guide) if you prefer the dashboard.

1. Install the [PlanetScale CLI](/concepts/planetscale-environment-setup).
...
You can use any name with lowercase, alphanumeric characters, and dashes or underscores.
...
If you're not using the CLI, you can get the exact values to copy/paste from your PlanetScale dashboard. In the dashboard, click on the the branch you want to connect to (we're using `main`), click "**Connect**", and select "**PHP**" from the language dropdown. Copy these credentials, and then skip to step 2 to fill them in.
...
3. For `MYSQL_ATTR_SSL_CA`, use our [CA root configuration doc](/concepts/secure-connections#ca-root-configuration) to find the correct value for your system. For example, if you're on MacOS, it would be:
...
We recommend connecting with username and password, but you can also open a quick connection with the PlanetScale proxy. You'll need the [PlanetScale CLI](https://github.com/planetscale/cli) for this option.
...
<InfoBlock type='note'>
  It's important to make sure that if you switch back to username and password credentials, you add the SSL check back. We're intentionally commenting it out instead of deleting it in case you switch back.
</InfoBlock>
...
PlanetScale does not support foreign key constraints, but we do support the concept of relationships with foreign keys, as shown in this example. For more information, check out our [Operating without foreign key constraints documentation](/learn/operating-without-foreign-key-constraints).
...
PlanetScale does not support foreign key constraints, but we do support the use of relationships with foreign keys, as shown in this example. For more information, check out our [Operating without foreign key constraints documentation](/learn/operating-without-foreign-key-constraints).
...
When you're reading to make more schema changes, you'll [create a new branch](/concepts/branching) off of your production branch. Branching your database creates an isolated copy of your production schema so that you can easily test schema changes in development. Once you're happy with the changes, you'll [open a deploy request](/concepts/deploy-requests). This will generate a diff showing the changes that will be deployed, making it easy for your team to review.
