...
To find your `deploy-request-number`, run:

```bash
pscale deploy-request deploy <DATABASE_NAME> <DEPLOY_REQUEST_NUMBER>
```

Copy the value from `NUMBER` and use that digit as your `DEPLOY_REQUEST_NUMBER`.

8. Merge your `development` branch into `main`:

```bash
pscale deploy-request deploy <DATABASE_NAME> <DEPLOY_REQUEST_NUMBER>
```

7. (Optional) Delete your just merged feature branch:
