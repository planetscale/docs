---
title: 'Connect a Golang application to PlanetScale'
subtitle: 'Spin up a PlanetScale MySQL serverless database in seconds and connect to a Golang application'
---

## Introduction

In this tutorial, you'll learn how to connect a Golang application to a PlanetScale MySQL database using a sample Golang starter app using GORM.
...
This guide will integrate [a simple Golang app](https://github.com/planetscale/golang-example.git) with PlanetScale that will display a list of products stored in the database. If you have an existing application, you can also use that.

1. Clone the starter Golang application:
...
Next, you need to set up your PlanetScale database and connect to it in the Golang application.
...
This tutorial uses `golang-example` for `DATABASE_NAME`, but you can use any name with lowercase, alphanumeric characters, and dashes or underscores.
...
That's it! Your database is ready to use. Next, let's connect it to the Golang application and then add some data.

## Connect to the Golang app

There are **two ways to connect** your Golang app to PlanetScale:
...
1. Open the `.env` file in your Golang app and update `DSN` as follows:
...
3. Open the `.env` file in your Golang app and update it as follows:
...
Now that you're connected let's add some data to see it in action. The sample application has an endpoint that you can use to run migrations to create your `categories` and `products` tables. It will seed your database with sample product and category data. You can find this in `main/main.go`.

Let's run those now.

1. First, start your Golang app with:
...
You can now navigated the [Golang products page](http://localhost:8080/products) to see the new record. 
...
![PlanetScale console insert and select example](/img/docs/golang-planetscale-console.png)

1. Go to your [PlanetScale dashboard](https://app.planetscale.com) and select your Golang database.
...
You can now refresh the [Golang products page](http://localhost:8080/products) to see the new record.
