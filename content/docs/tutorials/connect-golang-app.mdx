...
- [Golang](https://go.dev/doc/install) &mdash; This tutorial uses `v8.1`
...
2. Open the `.env` file in your Golang app and update `DSN` as follows:
...
You can also get these exact values to to copy/paste from your PlanetScale dashboard. In the dashboard, click on the database > "**Connect**" > "**Generate new password**" > "**General**" dropdown > "**Go**".
...
```
DSN="mysql://tcp(127.0.0.1:3306)/<DATABASE_NAME>?tls=true"
```
...
3. You can now see the products and categories at:
...
You can now refresh the [Golang homepage](http://localhost:8080) to see the new record. 
