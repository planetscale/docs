...
subtitle: 'Spin up a PlanetScale MySQL serverless database in seconds and connect to a Go application'
---
...
This guide will integrate [a simple Go (Golang) app](https://github.com/planetscale/golang-example.git) with PlanetScale that will display a list of products stored in the database. If you have an existing application, you can also use that.
...
For `DATABASE_NAME`, you can use any name with lowercase, alphanumeric characters, and dashes or underscores.
...
1. Open the `.env` file in your Go app and update `DSN` as follows:
...
PlanetScale does not support foreign key *constraints*, but we do support the use of relationships with foreign keys, as shown in this example. For more information, check out our [Operating without foreign key constraints](/learn/operating-without-foreign-key-constraints) documentation.  
...
![PlanetScale console insert and select example](/img/docs/Go-planetscale-console.png)
