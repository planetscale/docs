...
First, [sign up for a free PlanetScale account](https://auth.planetscale.com/sign-up). You can either do this on the PlanetScale website or [using the PlanetScale CLI](https://docs.planetscale.com/tutorials/planetscale-quick-start-guide#download-and-install-the-planetscale-cli).
...
For the `REGION` value, choose the region closest to you or your application's hosting location. You can find our regions and their slugs on the [Regions page](https://docs.planetscale.com/concepts/regions#available-regions). If you do not specify a region, your database will automatically be deployed to **US East - Northern Virginia**.
...
![Prisma Studio showing the Star table with one record](/img/docs/prisma_studio_example.png)
...
Currently, you're working on the branch `initial-setup`, which is branched off of the development branch, `inital-setup`. This branch is meant for applying schema changes and testing your migrations before you go to production.

Once you're happy with the `initial-setup` branch, you can roll it up to production. To do this, you first need to promote the `inital-setup` branch to production. This main branch is currently empty, but once it's set as the production branch, you'll be able to roll your changes from `initial-setup` into it.

To promote `inital-setup` to production:
...
![Example of the Star table schema in a deploy request on PlanetScale](/img/docs/prisma_deploy_request_schema.png)
...
Now that you have your Next.js application with a database in production on PlanetScale, you may want to deploy the entire application. You can deploy to Vercel in just a few easy steps by following our [Deploy to Vercel guide](/tutorials/deploy-to-vercel#deploy-to-vercel).
