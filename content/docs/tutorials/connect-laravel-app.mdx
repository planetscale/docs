...
This guide will integrate [a simple Laravel 9 app](https://github.com/planetscale/laravel-example) with PlanetScale that will display a list of stars stored in the database. If you have an existing application, you can also use that.
...
This tutorial uses `laravel_example` for `DATABASE_NAME`, but you can use any name with lowercase, alphanumeric characters, or underscores. We also permit dashes, but don't recommend them, as they may need to be escaped in some instances.
...
DB_PASSWORD=<PASSWORD> // outputted in the previous step
...
1. You may need to install the [MySQL command line client](/docs/concepts/planetscale-environment-setup) if you haven't already.
...
please read [Connecting to PlanetScale securely](/docs/reference/planetscale-security).
