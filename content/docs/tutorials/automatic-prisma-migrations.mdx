...
<InfoBlock type='note'>
  ...
  the recommendation to use `prisma db push` instead of {' '}`prisma migrate 
  dev` with shadow branches. Also, you previously needed to turn on the ability to automatically 
...
<InfoBlock type='note'>
  This step assumes you created a new PlanetScale database and the `main` branch 
  has not been promoted to production yet. You will need to create a new development branch if 
  the `main` branch has been promoted to production.
</InfoBlock>
