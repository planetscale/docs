...
<InfoBlock type="tip">
  To make non-blocking schema changes in PlanetScale, you'll first need a basic
  understanding of{' '}
...
<InfoBlock type="note">
  PlanetScale makes sure not to exhaust your resources; the deployment may be
  throttled to avoid any impact on production queries.
</InfoBlock>{' '}
...
<InfoBlock type="tip">
  A schema change is any change you make to the tables in your database
  environment created within the PlanetScale branch. (i.e., create, drop, and
  alter statements )
</InfoBlock>{' '}

<InfoBlock type="warning">
  You can only apply direct schema changes to the development branch, not to
  `main`.
</InfoBlock>{' '}
...
PlanetScale doesn't support `FOREIGN KEY` constraints. Learn more [in this blog post](http://code.openark.org/blog/mysql/the-problem-with-mysql-foreign-key-constraints-in-online-schema-changes) from one of our engineers.
