...
<InfoBlock type='note'>
  `pscale` can use the MySQL command-line client to quickly open an interactive shell for a
...
You're all set! Check out our [CLI reference page](/reference/planetscale-cli) to explore all that's possible with the PlanetScale CLI.
