...
Imagine you just deployed a deploy request that drops the `email` column on your `users` table. You refresh your production application in the browser only to discover, to your horror, that application is down. Thankfully, the signup page continues to work as expected, but you've found that some major parts of the application rely on that `email` column that no longer exists.
