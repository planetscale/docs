...
PlanetScale currently offers database deployment in multiple regions. Select the region closest to your application servers to reduce latency between your database and application. Deploy branches in the region closest to your location to help reduce latency when interacting with the database.

If you have questions about how to measure latency please [get in touch](https://planetscale.com/support) with PlanetScale.
...
<InfoBlock type="tip">
  Be sure to measure the latency from your location to each region.
...
Organization administrators can restrict branches to only being created in the same region as the one selected during database creation. To enable this setting, check the _Restrict region_ setting in the settings page for the database: `app.planetscale.com/<org>/<database>/settings`.
