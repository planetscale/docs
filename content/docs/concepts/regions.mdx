...
You may also add read-only regions to your production database. See our [Read-only regions documentation](/concepts/read-only-regions) for more information.
...
PlanetScale allows you to select the region for the [`main` branch](/concepts/branching) of your database during database creation. By default, all database branches created within this database will also be created in this region.
...
<InfoBlock type='note'>
  You cannot change the region of a branch.
</InfoBlock>
...
2. [Backup and dump](/reference/database) the original branch with:
```bash
...
```
4. If this is for a production branch, [promote the new branch](/reference/branch) to production:
```bash
pscale branch promote <DATABASE_NAME> <BRANCH_NAME>
```
5. Swap out the credentials in your app with the new branch.

It's important to note that this will require some downtime if doing this on a production branch, as the backup will not be up to date when you make the swap. You can temporarily block writes before doing the backup and restore them after the final credential swap to avoid data loss.
