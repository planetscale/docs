...
It's important to note that this will require some downtime if done on a production branch, as the backup will not be up to date when you make the swap. To avoid data loss, you can temporarily block writes before doing the backup, and restore them after the final credential swap.
