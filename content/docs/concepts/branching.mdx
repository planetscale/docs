...
### The `main` database branch

Every new PlanetScale database is created with a development branch named `main`.

This default development branch is intended for you to directly apply a schema, import data, and experiment with your database before promoting the branch to production. 

Once you are satisfied with the changes you've made to your `main` development branch, you can promote it to production. Going forward, you can continue to make new development branches off of this production branch whenever you need to make a schema change.
...
That said, you don't have to use the default `main` branch as your production branch. **Any development branch can be promoted to production**. It's also possible to run multiple production branches simultaneously. Keep in mind, PlanetScale does not provide data syncing between production branches.
