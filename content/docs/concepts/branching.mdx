...
<VideoBlock src='/img/docs/promote-a-branch-to-production.mp4' />
...
It's possible to run multiple production branches simultaneously, within [your plan's limits](/concepts/billing#planetscale-plans). Keep in mind, PlanetScale does not provide data syncing between production branches.

## Create a development branch

<VideoBlock src='/img/docs/create-new-branch.mp4' />
...
Development branches **will not** copy over production data, just the schema. To create a development branch with data from another branch, see the [Data Branching feature](/concepts/data-branching) secion.
...
5. (_Optional_) You can also create a new branch from the [PlanetScale CLI](/reference/branch) with:
...
If you are working in a team, the [deploy request](/concepts/deploy-requests) creates an opportunity for your teammates to review the changes you have made before they are deployed to production.
...
6. (_Optional_) You can also create a deploy request from the [PlanetScale CLI](/reference/deploy-request) with the following command:
...
<VideoBlock src='/img/docs/add-to-deploy-queue.mp4' />
...
3. (_Optional_) You can also run the command below to see the [schema diff in the PlanetScale CLI](/reference/deploy-request):
...
3. (_Optional_) You can also run the following command with the [PlanetScale CLI](/reference/deploy-request):
...
<InfoBlock type='note'>
  Only <a href='/concepts/access-control#organization-administrator'>Organization Administrators</a> have permission to
