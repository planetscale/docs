...
Encrypted data is transmitted to PlanetScale databases through two major paths:
 ...
 - PlanetScale [Connection Strings](/concepts/connection-strings) require a successfull establishiment of a TLS session before any SQL commands can be issued.
...
### Security Training todo

During onboarding (within 30 days of their hire date) and annually thereafter, all employees must
complete security awareness training covering company security policies and procedures. The
training courses are designed to assist employees in identifying and responding to social
engineering attacks and in avoiding inappropriate security practices.
...
Background checks are performed on new hires during onboarding (within 60 days of their hire date)
as permitted by local law. Issues, if any, are resolved.

## Vulnerability Disclosure

### In Scope

PlanetScale is actively seeking vulnerability reports from the following components that make up the product and it's Production Environment:
...
If you believe you have discovered a security vulnerability in a PlanetScale product or it's Production Environment, please let us know immediately.
