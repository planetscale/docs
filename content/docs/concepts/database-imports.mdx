...
PlanetScale requires that either `expire_logs_days` or `binlog_expire_logs_seconds` are set to a valid value. If both of these values are set, the value of `binlog_expire_logs_seconds` takes precedence over `expire_logs_days`.
