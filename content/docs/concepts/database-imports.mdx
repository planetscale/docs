...
To ensure that we can migrate your data to PlanetScale with zero downtime, we check your database for some required configuration values. 
...
| binlog_expire_logs_seconds | > ( 2* 24 * 3600) | [Documentation](https://dev.mysql.com/doc/refman/8.0/en/replication-options-binary-log.html#sysvar_binlog_expire_logs_seconds)|

### A note about bin log retention periods
Either  `expire_logs_days` or `binlog_expire_logs_seconds` needs to be set to a valid value.
If both of these values are set,  the value of `binlog_expire_logs_seconds` takes precedence over `expire_logs_days`


## List the values for your database server

Run this query in a mysql console connected to your database server to check their values. 
