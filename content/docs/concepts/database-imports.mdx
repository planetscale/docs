...
1. If connecting with a username & password, try to connect with the `mysql CLI` with the connection parameters to ensure that you can connect.


## Troubleshooting Compatibility issues

PlanetScale runs vitess and has to ensure that we can perform data migrations at scale, you might run into a few issues
importing your existing database into PlanetScale.

Here's a few common errors you may run into while importing or connecting to an external database.
...
Please take a look at our help document on how your application can [operate without foreign keys](https://docs.planetscale.com/tutorials/operating-without-foreign-keys)

### No unique key
PlanetScale requires that all tables have a unique, not-null key that remains unchanged during the migration. 
Please take a look at the offical [MySQL documentation about Primary Keys](https://dev.mysql.com/doc/refman/8.0/en/partitioning-limitations-partitioning-keys-unique-keys.html)
pkg/handlers/lintsnapshot/handler.go
...
### Unsupported feature
You might see this error if your table requires the `MyISAM` engine.
PlanetScale does not support the `MyISAM` engine.
Please consult the official [MySQL documentation about alternative storage engines](https://dev.mysql.com/doc/refman/8.0/en/storage-engines.html)
