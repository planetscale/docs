...
In other words, any changes made to your external database will also reflect in your PlanetScale database. For example, if a user makes a new comment on your blog application while you're importing, your PlanetScale database will be updated as well. This ensures you **won't experience any data loss** while going through the import process.

While in Replica mode, if you make any changes to your PlanetScale database, those will not be replicated back to your external database. To replicate both ways, you need to turn on "Primary mode".
...
Your database has been fully imported into PlanetScale and is ready to use! Once you detach, we automatically [promote the branch to production](/concepts/branching#branch-promotion). Production branches are highly available and protected from direct schema changes.
