...
During step one, your external database is imported into your PlanetScale database. The connection remains open during this phase and in `read-only` mode.
...
While in Replica mode, we'll continue watching your external database and automatically update your PlanetScale database with any changes you make.

Once you're ready for PlanetScale to become your primary database, click on "Enable read-write mode" to head to the next step.

### Step 3: Primary mode 

![Step 3 of database import - Primary mode](/img/docs/database-imports-step-3.png)

During this step, PlanetScale becomes the primary database instead of your external database and is fully ready to run, as `read-write` mode is enabled. If you're happy with everything, you can now click on the "**Detach external database**" to finalize the import.

### Step 4: Detach external database

After you detach the external database, PlanetScale's connection to that database will be closed and all connection information will be deleted from PlanetScale. Your database has now been fully imported into PlanetScale and is ready to use!
