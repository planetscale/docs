...
PlanetScale allows you to [create and manage passwords](/concepts/connection-strings) for each branch of your database. PlanetScale passwords can be created with one of three roles:
...
\* *This does not apply to production branches, as we [do not allow direct DDL](/learn/how-online-schema-change-tools-work) on production branches, even if your password has the `Admin` role.*
...
![PlanetScale password roles](/img/docs/password_roles.png)
...
<InfoBlock type="note">
The default role for all passwords created by the **Connect** button is administrator.
Please head to &lt;organization&gt;/&lt;database&gt;/settings/passwords page to create passwords with custom roles.
</InfoBlock>
...
<InfoBlock type="note">
If your pscale CLI version is less than 0.94.0, please upgrade your installation by following [this document](/concepts/planetscale-environment-setup)
</InfoBlock>
