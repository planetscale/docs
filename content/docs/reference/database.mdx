...
| `restore-dump <DATABASE_NAME> <BRANCH_NAME>` | `--dir <DIRECTORY_NAME>`*, `--local-addr <ADDRESS>` | Restore the specified database from a local dump directory |
...
| `--region` | Specify the [region](/concepts/regions) of the new database. Default is `us-east`. | `create` |
