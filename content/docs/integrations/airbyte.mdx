...
- **Connector Documentation URL**: https://docs.planetscale.com/integrations/airbyte
...
![Airbyte new PlanetScale connector](/img/docs/airbyte-new-connector.png)
...
- **Shards**: Sharding is only supported on our Enterprise plan. Please [reach out to us](https://planetscale.com/contact) for more information.

![Airbyte - PlanetScale source setup](/img/docs/airbyte-planetscale-source-setup.png)
...
![Airbyte - PlanetScale replication frequency](/img/docs/airbyte-replication-frequency.png)

<InfoBlock>
  <strong>Important:</strong> The sync frequency will affect your PlanetScale billing. The rows read during sync are counted toward your database rows read, so if you choose "Full refresh", you will be billed for full table reads of all tables selected every time the sync runs.
  <br/>
  You can find more information about your plan's read limits in our <a href="/concepts/billing#planetscale-plans">Billing documentation</a>.
</InfoBlock>
...
5. *(Optional)* Choose your destination stream prefix.
...
- **Incremental** &mdash; Incremental sync pulls *only* the data that has been modified/added since the last sync. We use [Vitess VStream](https://vitess.io/docs/13.0/concepts/vstream/) to track the stopping point of the previous sync and only pull any changes since then.
- **Full refresh** &mdash; Full refresh pulls *all* data at every scheduled sync frequency. This will lead to a higher rows read count than Incremental sync. For more information, see the [billing section of this doc](#billing).

![Airbyte - PlanetScale stream sync](/img/docs/airbyte-streams.png)
...
PlanetScale Connect is available on all of our [free and paid plans](/concepts/billing#planetscale-plans) during the beta period.

Every Airbyte connection sync will count toward [your plan's `rows read`](/concepts/billing#planetscale-plans). When setting up your connection, make sure you're aware of the impact on `rows read`, and choose [a synchronization schedule](#choose-your-sync-frequency) that you're comfortable with.
...
![Airbyte - PlanetScale disconnection](/img/docs/airbyte-disconnect.png)
