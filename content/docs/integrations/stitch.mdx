...
PlanetScale's Stitch tap outputs records and metadata to stdout so that the `http tap` can import them
into Stitch via [Stitch Import API](https://www.stitchdata.com/docs/developers/import-api/)

1. Signup for a StichData.com account at : https://app.stitchdata.com/signup
...
![Airbyte new PlanetScale connector](/img/docs/stitch-add-integration.png)
4. On the next screen, configure your integration name & destination.
![Airbyte new PlanetScale connector](/img/docs/stitch-configure-integration.png)
5. Once the integration is created, save the access token for use with the PlanetScale tap.
 ![Airbyte new PlanetScale connector](/img/docs/stitch-api-token.png)

### Step 2 : Configure the PlanetScale Stitch Tap
1. Click on the database and branch you want to connect to.
2. Click "Connect", select "Stitch source" from the "Connect with" dropdown.
3. Leave this tab open, as you'll need to copy these credentials shortly.
![Airbyte new PlanetScale connector](/img/docs/stitch-source-config.png)
4. Save the contents of `source.json` as a file on your local file system and save it as `source.json`. This will now act
...
1. Clone this repository onto the machine running the workflow: https://github.com/planetscale/singer-tap
2. Save the schema for your PlanetScale database. 

```
go run cmd/singer-tap/main.go --config source.json  --discover > schema.json
```

3. Select the schema elements you want synced. **TODO: We need do something about this**

4. Sync your PlanetScale database to Stitch by running the following command

```
go run cmd/singer-tap/main.go --config source.json  --catalog schema.json | go run cmd/htt-tap/main.go  --api-token $(cat access_token)
```
...
6. Any state outputted by the PlanetScale Tap will be saved and you can look at the logs for the location.
example:
...
7. In Stitch, you should see that it loaded `21` records to be replicated. 
![Airbyte new PlanetScale connector](/img/docs/stitch-import-done.png)
