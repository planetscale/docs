...
Make sure to first read [How Online Schema Change tools work](/learn/how-online-schema-change-tools-work).
...
- **Copy** &mdash; opens a transaction with `CONSISTENT SNAPSHOT` while atomically grabbing the `GTID` at time of te transaction. Begins streaming rows from the table.
...
`Vitess`, like `fb-osc`, creates a puncture experience, but instead of doing a two-step rename, it blocks writes to the original table by virtue of `VTGate`, its proxy component. When it is done processing the backlog, it unblocks writes. The users and apps experience unexpected query errors.
...
`pt-osc`, `fb-odc` and `gh-ost` require an `ALTER TABLE` statement. This statement is pre-evaluated by the user (or an ORM) and must be valid. The tools have no notion of the purpose nor semantics of the change.
