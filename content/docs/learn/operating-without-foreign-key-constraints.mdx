...
We believe the advantages of Online DDL such as [branching](/concepts/branching), developer-owned schema changes and deployments, [non-blocking schema changes](/concepts/nonblocking-schema-changes), etc., and the advantages of sharding as means of unlimited scaling, outweigh the `FOREIGN KEY` constraints benefits.
