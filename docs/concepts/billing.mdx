...
{% callout %}
  You are limited to __one free database per organization__. Free databases may be
...
{% callout %}
  Plans are priced __per database__. The total charge for a PlanetScale _database_ includes the charge for
  usage on __database branches__. In other words, the total `reads`, `writes`, and storage used by each branch of
...
{% callout %}
  PlanetScale generates both current and past invoices. Even for the __free__ plan! You can see the cost had you not
