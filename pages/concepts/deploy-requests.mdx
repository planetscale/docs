...
8. Here, you also have the option to enable [**Gated Deployments**](#gated-deployments), which gives you the chance to control exactly when the migration cuts over. You'll see an "**Auto-apply changes**" checkbox, which is checked by default. If you uncheck this, you will get the option to apply the changes once the schema changes are complete. If you leave it checked, it will auto-deploy as soon as it's ready.
9. On the "**Summary**" tab, click "**Add changes to deploy queue**". The deployment will begin or be queued if there are other pending deployments. You also have a chance to enable Gated Deployments in this step.
...
11. After you deploy, you have **30 minutes to "undo"** it using our [Rewind feature](#revert-a-schema-change).
